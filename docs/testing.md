...
- [ ] `eslint-plugin-jest-dom` (and later `eslint-plugin-testing-library`) on `*.test.tsx` when suites exist — companion lint from [jest-dom install notes](https://github.com/testing-library/jest-dom#installation) / [ecosystem](https://testing-library.com/docs/dom-testing-library/install/#ecosystem); not required for matchers to work
