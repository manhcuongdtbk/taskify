...
| `index.ts` (handler)              | **Vitest + mocks** (`auth`, Prisma, `revalidatePath`) when the suite is written | Same pattern as [`lib/create-audit-log.test.ts`](../lib/create-audit-log.test.ts). [`create-board/index.test.ts`](../actions/create-board/index.test.ts) is the first handler suite. Coverage still excludes `actions/**/index.ts` until more handlers land. |
