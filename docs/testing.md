...
| Match **return shape** with the factory name — `cardFactory` → `Card`; `cardWithListFactory` → `CardWithListTitle`; `listFactory` → `List`; `listWithCardsOrderedByOrderAscFactory` → `ListWithCardsOrderedByOrderAsc`                     | One factory for both shapes, or a name that hides whether relations are nested                                                                                                                                                                                                     |
