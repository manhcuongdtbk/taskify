...
| Nested rows **on the return type**: Fishery `associations` (`cards` on list-with-cards). Pair denormalized fields with `transient` when needed (`auditLogFactory` + `card`)                                           | Use prisma-fabbrica (or a second factory lib) for UI/MSW-only data                                                                                            |
