...
- [ ] Playwright for critical flows (auth, board, billing) — `e2e/*.spec.ts` only (never `*.test.*`; only E2E tool; no Cypress). Include **async RSC**: create-board gate (navbar + org tile — popover vs Pro modal). Next’s [Vitest guide](https://nextjs.org/docs/app/guides/testing/vitest) — do not emulate that in jsdom.
...
- **Booting the App Router in Vitest** (`page.tsx` / `layout.tsx` HTTP, cookies, RSC) — Next’s [Vitest guide](https://nextjs.org/docs/app/guides/testing/vitest) says async Server Components are not a Vitest job. jsdom and Browser Mode do not boot the App Router. Do **not** paper over that with a jsdom RSC walker. That proof waits on Playwright ([TODO](#todo)). **Calling an exported Route Handler `GET`/`POST` with mocked `auth` / Prisma is Vitest** — see [Route Handlers](#route-handlers-appapi) below. **Client children of an async Server Component:** mock the async child in the parent suite (e.g. `CreateBoardTrigger` in the navbar/tile) and assert the sync chrome; do not `render()` the async component.
...
| `index.ts` (handler)              | **Vitest + mocks** (`auth`, Prisma, `revalidatePath`) when the suite is written | Same pattern as [`lib/create-audit-log.test.ts`](../lib/create-audit-log.test.ts). Handler suites: [`create-board/index.test.ts`](../actions/create-board/index.test.ts), [`delete-board/index.test.ts`](../actions/delete-board/index.test.ts). Coverage still excludes `actions/**/index.ts` until more handlers land. |
...
3. **Need real SQL / relations** → integration against a test DB (later; blog part 3 / Prisma integration docs) — not default for app helpers. First candidate when that suite exists: [`incrementAvailableCount`](../lib/organization-limit.ts) `createMany` `skipDuplicates` inside `$transaction` ([`prisma.md`](./prisma.md)).
