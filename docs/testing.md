...
| Do                                                                                                                                                                                                                    | Don’t                                                                                                                                                         |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| One file per entity under [`lib/testing/factories/`](../lib/testing/factories/); `.build({ … })` **inside each `test`** (ESLint)                                                                                      | Nest another entity’s factory in a sibling file; hand-roll parallel `make*`; share `const x = fooFactory.build()` at module / `describe` / `beforeEach` scope |
| Match factory name → return shape — `cardFactory` → `Card`; `cardWithListTitleFactory` → `CardWithListTitle`; `listFactory` → `List`; `listWithCardsOrderedByOrderAscFactory` → `ListWithCardsOrderedByOrderAsc`      | One factory for both shapes, or a name that hides nested relations                                                                                            |
| `sequence` ids (`card_${sequence}`); prefer a real related row for required FKs when cheap (`cardFactory` builds a List for `listId`). Placeholder FKs only when the parent is out of scope (`listFactory` `boardId`) | Hardcode magic ids/dates every suite couples to                                                                                                               |
| Assert **built fields** (`card.id`, `card.title`)                                                                                                                                                                     | Hardcode sequence numbers unless you deliberately override                                                                                                    |
| Nested rows **on the return type**: Fishery `associations` (`cards` on list-with-cards). Pair denormalized fields with `transient` when needed (`auditLogFactory` + `card`)                                           | Invent a second ORM factory stack (fabbrica) for UI/MSW-only data                                                                                             |
| `rewindSequence` / `rewind*Factory` in `beforeEach` when asserting `card_1`-style ids                                                                                                                                 | Mutable shared builds across tests                                                                                                                            |
| `.build` for in-memory / MSW; `.create` / DB factories only when a real DB suite lands                                                                                                                                |                                                                                                                                                               |
