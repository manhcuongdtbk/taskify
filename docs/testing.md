...
| Match **return shape** with the factory name — `cardFactory` → `Card`; `cardWithListTitleFactory` → `CardWithListTitle`; `listFactory` → `List`; `listWithCardsOrderedByOrderAscFactory` → `ListWithCardsOrderedByOrderAsc`                | One factory for both shapes, or a name that hides whether relations are nested                                                                                                                                                                                                     |
...
| **Associations** for nested models (import peer factories; pass `cards` into `listWithCardsOrderedByOrderAscFactory`); **`afterBuild`** to keep derived FKs in sync after DeepPartial overlays                                             | Assume nested overrides leave sibling FKs correct without an after-build sync; redefine another entity’s factory in the wrong file                                                                                                                                                 |
