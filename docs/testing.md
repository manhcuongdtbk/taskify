...
2. **All-files floor** — [`vitest.config.mts`](../vitest.config.mts) `coverage.thresholds` (**99%** statements / branches / functions / lines). That is a **global minimum on the coverage bucket** (`coverage.include` minus `coverage.exclude`), not “every file in the repo is 100%” and not a per-file gate. The bucket is `lib` / `components` / `actions` / `hooks` / `stores` / `constants` / `providers/query-provider.tsx`. Types-only files (`**/*.types.ts`, `**/types.ts`) are excluded. **Untested Action handlers stay excluded** (`actions/!(create-board|delete-board)/index.ts`). Drop a path from that glob when its mocked Vitest suite lands. **`app/` is not in the bucket:** `page` / `layout` / `route` need a running Next app (no E2E yet); `app/**/_components` still use colocated 100% via `pnpm test:coverage:paths` without counting toward the 99%. CI: [`pnpm test:coverage`](../.github/workflows/ci.yml).
...
| `index.ts` (handler)              | **Vitest + mocks** (`auth`, Prisma, `revalidatePath`) when the suite is written | Same pattern as [`lib/create-audit-log.test.ts`](../lib/create-audit-log.test.ts). In the coverage bucket today: [`create-board/index.ts`](../actions/create-board/index.ts), [`delete-board/index.ts`](../actions/delete-board/index.ts). Other `actions/**/index.ts` stay excluded until their suites land. |
