...
- **Cover executable helper logic.** Keep colocated `*.test.*` for helpers with real branches. `lib/testing/**` (and `lib/auth/**`) are excluded from the All-files 99% bucket in [`vitest.config.mts`](../vitest.config.mts) as test infrastructure / mocked wrappers — location and peer suites still matter; they just don’t count toward that floor.
...
The Vitest backlog freeze is **done**. Product work is no longer blocked by that freeze.
...
Live coverage numbers are not stored in git — regenerate with `pnpm test:coverage` and open gitignored [`coverage/`](../coverage/) (see [Coverage reports](#coverage-reports-vitest-output)).
...
Defaults: [Vitest coverage](https://vitest.dev/guide/coverage.html) · `reportsDirectory` default `./coverage`. Re-run to refresh; do not commit `coverage/`. All-files floors: `coverage.thresholds` in [`vitest.config.mts`](../vitest.config.mts).
...
| `index.ts` (handler)              | **Vitest + mocks** (`getOrgAuth`, Prisma, `revalidatePath`) when the suite is written | Same pattern as [`lib/create-audit-log.test.ts`](../lib/create-audit-log.test.ts). `createSafeAction` calls `getOrgAuth` automatically; tests mock `@/lib/auth/get-org-auth`. Handlers are in the coverage bucket (create/delete/update board, create/copy/update/delete list, create/copy/update/delete card, update list/card order, stripe-redirect). |
...
- **Auth fixtures live under `lib/testing/auth/`** — identity/session test helpers (`orgAuth`, `authUserFactory`) are auth fixtures, not domain entity factories. Keep model/entity Fishery files under [`lib/testing/factories/`](../lib/testing/factories/).
