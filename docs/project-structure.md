...
| `scripts/` | One-off maintenance / codegen scripts | Adopted | Keep out of `app/`; e.g. `check-route-export-names.ts` (`pnpm lint:routes` / `pnpm lint:routes:fix`) — route-mirrored export names + `PageProps`/`LayoutProps`/`RouteContext` — see [`conventions.md`](./conventions.md#route-mirrored-pagelayout-names) · [`nextjs.md`](./nextjs.md#route-props-helpers-pageprops--layoutprops--routecontext) |
