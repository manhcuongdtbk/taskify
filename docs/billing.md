...
Navbar is only Create / plus — no remaining count, no `?` Hint. The org tile shows remaining copy and the Hint (copy switches to Pro messaging when `isPro`).
...
| Item                                  | Why                                                                                 | Where                                                                                                                                                    |
| ------------------------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~Board-list Hint~~                   | ~~Still describes the Free-plan 5-board limit when the organization is on **Pro**~~ | Done — Hint + remaining copy switch to Pro messaging via [`lib/board-limits/create-board-limit-copy.ts`](../lib/board-limits/create-board-limit-copy.ts) |
