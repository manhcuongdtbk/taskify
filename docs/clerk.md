...
Learning reference for **Clerk** authentication and **Organizations** in this Next.js App Router app. Prefer [Clerk docs](https://clerk.com/docs) for the SDK version in `package.json`. Repo Clerk skills under `.claude/skills/clerk*` can help with setup, Organizations, and Next.js patterns.
...
In prose, write **authentication** (not “auth”) and **organization** (not “org”). Keep Clerk identifiers such as `auth()`, `useAuth`, and `orgId`.
...
## TODO — follow Clerk / authentication recommendations more closely
...
- [ ] **Authorization checklist with Next.js** — every Server Action / protected Route Handler verifies `userId` / `orgId` (and resource ownership) — shared with [`nextjs.md`](./nextjs.md) security TODO
