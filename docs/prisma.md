...
- **Next.js singleton Client** in `lib/prisma/client.ts` with `@prisma/adapter-pg` and `DATABASE_URL` ([Prisma + Next.js](https://www.prisma.io/docs/guides/frameworks/nextjs)) — no barrel `index.ts`; import the file you need (`@/lib/prisma/client` vs `@/lib/prisma/query-options/<model>` vs `@/lib/prisma/lock-for-update`)
...
- **Sequential writes in interactive `$transaction`** — adapter-pg uses one connection per callback ([Transactions](https://www.prisma.io/docs/orm/prisma-client/queries/transactions)). [`updateCardOrder`](../actions/update-card-order/index.ts) `await`s each `tx.card.update`; `Promise.all` on `tx` can overlap queries on that connection.
- **List/card find-then-create** — [`createList`](../actions/create-list/index.ts) / [`createCard`](../actions/create-card/index.ts) / [`copyList`](../actions/copy-list/index.ts) / [`copyCard`](../actions/copy-card/index.ts) run the org-scoped parent lookup, parent-row `SELECT … FOR UPDATE` ([`lock-for-update.ts`](../lib/prisma/lock-for-update.ts)), next-order read, and insert on one interactive `tx`. Concurrent creates/copies on the same board (lists) or list (cards) wait on that lock so `max(order) + 1` does not collide. A missing parent/source **returns** `{ created: false }` / `{ copied: false }` (do not throw) so “not found” is not swallowed as “Failed to create/copy.” Do not add a unique on `(boardId, order)` / `(listId, order)` — reorder can still write duplicate orders. `createAuditLog` stays outside the callback.
- **Organization board-limit writes** — The stored `OrganizationLimit.count` tracks the **actual number of open boards** regardless of plan. Both Free and Pro creates/deletes lock the org’s limit row (`SELECT … FOR UPDATE`), `COUNT` that org’s `Board` rows, and then sync/write the live number (at-cap as-is; after successful create/delete to match reality). That keeps the counter aligned when it had drifted. [`prisma/migrations/20260818120000_backfill_organization_limit_count`](../prisma/migrations/20260818120000_backfill_organization_limit_count/migration.sql) backfills existing rows. **Free creates** refuse when `COUNT(boards) >= maxBoards` (cap check runs under the limit-row lock); the at-cap stored-count **sync** must **commit** — [`createBoard`](../actions/create-board/index.ts) returns the limit error after `$transaction` resolves; throwing inside the callback rolls the heal back. **Pro creates** sync the stored counter from the live `COUNT` after the insert. **Deletes** sync `COUNT` after the board row is gone — no plan check needed. A failed insert/delete rolls the counter change back. Plan access uses [`isProOrganization`](../lib/subscription.ts) on that same `tx` (not a pre-transaction `checkSubscription()`). `withOrganizationLimitLock` (and any helper used by create/delete) takes `orgId` from the Action and the interactive-transaction client (`tx`) — no `auth()` and no default global Client, because `FOR UPDATE` is released at statement end outside a transaction. `Board.orgId` is indexed for that `COUNT`. The lock target is inserted with `createMany` `skipDuplicates` (`ON CONFLICT DO NOTHING`) so a unique collision cannot abort the interactive transaction. Official `P2002` handling (when a query must throw): `instanceof Prisma.PrismaClientKnownRequestError` and `code === "P2002"` — [Handling exceptions and errors](https://www.prisma.io/docs/orm/prisma-client/debugging-and-troubleshooting/handling-exceptions-and-errors) · [Error reference (`P2002`)](https://www.prisma.io/docs/orm/reference/error-reference#p2002); catch **outside** `$transaction`, or isolate the throwing query in a nested `$transaction` (savepoint, Prisma 7.5+). No repo `P*` helper until a second call site needs one ([`project-structure.md`](./project-structure.md) `lib/prisma/errors/`). The user-facing cap copy is [`lib/board-limits/free-board-limit.ts`](../lib/board-limits/free-board-limit.ts) (shared Action / FormPopover string, not a Prisma `P*` code). [`lib/board-limits/organization-limit.ts`](../lib/board-limits/organization-limit.ts) · [`actions/create-board/index.ts`](../actions/create-board/index.ts) · [`actions/delete-board/index.ts`](../actions/delete-board/index.ts)
