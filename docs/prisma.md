...
- **Copy find-then-create** — [`copyList`](../actions/copy-list/index.ts) / [`copyCard`](../actions/copy-card/index.ts) run the org-scoped lookup, next-order read, and insert on one interactive `tx`. A missing source **returns** `{ copied: false }` (do not throw) so “List/Card not found” is not swallowed as “Failed to copy.” `createAuditLog` stays outside the callback.
