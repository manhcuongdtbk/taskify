...
| **Open when**   | Changing schema, Client setup, migrations, org-scoped queries, **Prisma Client error helpers** (`lib/prisma/errors/`), or **shared Prisma query args / GetPayload types** (`lib/prisma/query-options/<model>.ts`) |
...
- **Next.js singleton Client** in `lib/prisma/client.ts` with `@prisma/adapter-pg` and `DATABASE_URL` ([Prisma + Next.js](https://www.prisma.io/docs/guides/frameworks/nextjs)) — no barrel `index.ts`; import the file you need (`@/lib/prisma/client` vs `@/lib/prisma/query-options/<model>` vs `@/lib/prisma/errors/unique-constraint`)
...
- **Organization board-limit writes** — Free-plan creates take a slot with `updateMany` `count < maxBoards` (or `create` `count: 1`) in the same `$transaction` as `board.create` so concurrent creates cannot exceed the cap; a failed insert rolls the increment back. `incrementAvailableCount` takes `orgId` from the Action (no `auth()` while the row is locked). Concurrent first-row creates retry on Prisma `P2002` via [`isUniqueConstraintError`](../lib/prisma/errors/unique-constraint.ts) (see **Prisma Client `P2002`** below). The user-facing cap is [`lib/errors/free-board-limit.ts`](../lib/errors/free-board-limit.ts) (domain `Error`, not a Prisma `P*` code). Decrement still uses `updateMany` + `gt: 0`. [`lib/organization-limit.ts`](../lib/organization-limit.ts) · [`actions/create-board/index.ts`](../actions/create-board/index.ts)
- **Prisma Client `P2002`** — [`lib/prisma/errors/unique-constraint.ts`](../lib/prisma/errors/unique-constraint.ts) is our named check for unique-constraint failures. Official pattern (Prisma 7): `instanceof Prisma.PrismaClientKnownRequestError` and `code === "P2002"` — [Handling exceptions and errors](https://www.prisma.io/docs/orm/prisma-client/debugging-and-troubleshooting/handling-exceptions-and-errors) · [Error reference (`P2002`)](https://www.prisma.io/docs/orm/reference/error-reference#p2002). Catch-and-retry on concurrent unique races: [unique key constraint errors on upserts](https://www.prisma.io/docs/orm/reference/prisma-client-reference#unique-key-constraint-errors-on-upserts) · [`connectOrCreate`](https://www.prisma.io/docs/orm/reference/prisma-client-reference#connectorcreate). Prisma does **not** ship a helper, duck-type `{ code: "P2002" }`, or `Error.cause` walking — those extras are ours. [NestJS + Prisma 7 error handling](https://www.prisma.io/blog/nestjs-prisma-error-handling-7D056s1kOop2) maps `P2002` to HTTP 409; that is a different job (REST filters), not Action retry. Domain Action errors stay in `lib/errors/`.
