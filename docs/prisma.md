...
- **List/card find-then-create** — [`createList`](../actions/create-list/index.ts) / [`createCard`](../actions/create-card/index.ts) / [`copyList`](../actions/copy-list/index.ts) / [`copyCard`](../actions/copy-card/index.ts) run the org-scoped parent lookup, next-order read, and insert on one interactive `tx`. A missing parent/source **returns** `{ created: false }` / `{ copied: false }` (do not throw) so “not found” is not swallowed as “Failed to create/copy.” `createAuditLog` stays outside the callback.
