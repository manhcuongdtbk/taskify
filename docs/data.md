...
2. **Remote data** loads with TanStack Query via a **resource factory** — [`lib/api/card.ts`](../lib/api/card.ts) (`cardQueries.detail` / `cardQueries.auditLogs`) → our Route Handlers under `app/api/cards/…` today (same pattern later for other origins).
