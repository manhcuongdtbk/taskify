...
- **Route Handlers** — webhooks and client-facing JSON the modal needs; not the primary mutation style. Handler throws vs HTTP status: [`nextjs.md`](./nextjs.md). Card detail / audit-log routes call [`notFound()`](https://nextjs.org/docs/app/api-reference/functions/not-found) when the card is missing (or another org); audit-logs load the card first in an interactive `$transaction`, skip `findMany` when missing, then `notFound()` **outside** the transaction. Query factories parse a card object (not `| null`); the modal treats a missing card as `isError`, keeps activity on a skeleton until the card succeeds, then hides activity on card error, and remounts header / description / actions / activity when the card id changes ([`card-modal`](../components/modals/card-modal/index.tsx))
