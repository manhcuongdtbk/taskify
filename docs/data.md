...
| After success, refresh **TanStack Query** cache              | `queryClient.invalidateQueries` on the client using keys from [`lib/api/`](../lib/api/) factories                                                                                                                                                                                                                                                                                                                                                                                            | Card modal → one `cardQueries.byId(id)` (prefix-scopes `.detail` + `.logs`)                                                                      |
...
2. **Remote data** loads with TanStack Query via a **resource factory** — [`lib/api/card.ts`](../lib/api/card.ts) (`cardQueries.detail` / `cardQueries.logs`) → our Route Handlers under `app/api/cards/…` today (same pattern later for other origins).
