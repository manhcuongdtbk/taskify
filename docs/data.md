...
- [x] **TanStack Query factories** — `queryOptions` in [`lib/api/<resource>.ts`](../lib/api/) (card modal done). Remaining: provider nesting polish — see [TanStack Query](#tanstack-query-client-only)
...
| After success, refresh **TanStack Query** cache              | `queryClient.invalidateQueries` on the client using keys from [`lib/api/`](../lib/api/) factories                                                                                                                                                                                                                                                                                                                                                                                            | Card modal → one `cardQueries.byId(id)` (prefix-scopes `.detail` + `.auditLogs`)                                                                    |
...
| **Client factories** (keys + `queryFn`, later Query mutations)  | [`lib/api/<resource>.ts`](../lib/api/) — SoT for layout below. **Word “factory”:** [`vocabulary.md`](./vocabulary.md) (TanStack does not define it; official helper is [`queryOptions`](https://tanstack.com/query/v5/docs/framework/react/guides/query-options)) |
| **Folder map** (`lib/api` vs `app/api`, avoid `queries/`)       | [`project-structure.md`](./project-structure.md) (paths only — **why** stays here)                                                                                                                                                                                |
...
2. **Remote data** loads with TanStack Query via a **resource factory** — [`lib/api/card/`](../lib/api/card/) (`cardQueries.detail` / `cardQueries.auditLogs`) → our Route Handlers under `app/api/cards/…` today (same pattern later for other origins).
...
| `lib/api/` vs `app/api/`                            | **Both, different jobs** | `app/api/` = Next **Route Handlers** (HTTP we expose). `lib/api/` = **client** factories that call remotes (often our handlers, sometimes not).                                                                                                                                                                                                                                                                   |
...
**Example in tree today:** [`lib/api/card/`](../lib/api/card/). Provider: `providers/query-provider.tsx`.
