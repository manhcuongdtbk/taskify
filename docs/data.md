...
- **Default write path** — Server Actions under `actions/` with Zod schemas; `createSafeAction` + `ActionState` (`SchemaActionErrors` ∩ `HandlerActionResult`: `fieldErrors` / `formErrors` from the schema, `serverError` / `data` from the handler) — [`conventions.md` → Action validation messages](./conventions.md#action-validation-messages-zod). `createSafeAction` always verifies org auth via [`getOrgAuth()`](../lib/auth/get-org-auth.ts) before calling the handler; the handler receives `OrgAuth` (`{ userId, orgId }`) as its second argument
- **UI may be stale; writes re-check** — rendered UI is not the lock; Actions / protected Route Handlers re-read session, tenant, ownership, and entitlements on that request — [UI may be stale; writes re-check](#ui-may-be-stale-writes-re-check)
- **Client read cache** — TanStack Query + `lib/tanstack-query/fetcher.ts` for card modal (and similar client islands). Keep the `useQuery` result object and branch `isPending` → `isError` → `data` ([Queries](https://tanstack.com/query/v5/docs/framework/react/guides/queries)); do not destructure those fields (loses the status union) or use `!data` as loading (failed fetches look like skeletons). [`createQueryClient`](../lib/tanstack-query/client.ts) skips retries on HTTP 4xx (except 408/429) and Zod parse failures ([Query Retries](https://tanstack.com/query/v5/docs/framework/react/guides/query-retries))
...
- [x] **TanStack Query factories** — `queryOptions` in [`lib/tanstack-query/resources/<resource>/`](../lib/tanstack-query/resources/) (card modal done). Remaining: provider nesting polish — see [TanStack Query](#tanstack-query-client-only)
...
- [ ] **Real optimistic UI (`useOptimistic`)** where latency hurts — start with board list DnD ([`lists-container/index.tsx`](<../app/(platform)/(dashboard)/board/[boardId]/_components/lists-container/index.tsx>)); keep confirmed local mirrors for simple post-success title sync ([Client mirrors](#client-mirrors-after-server-actions-not-always-optimistic))
...
- [x] **404 instead of a 200 `null` body** — [`app/api/cards/[cardId]`](../app/api/cards/%5BcardId%5D/route.ts) and [`audit-logs`](../app/api/cards/%5BcardId%5D/audit-logs/route.ts) call [`notFound()`](https://nextjs.org/docs/app/api-reference/functions/not-found) when the card is missing (or another org); audit-logs uses an interactive `$transaction`: load the card first, skip `findMany` when it is missing, then `notFound()` **outside** the transaction (a throw inside `$transaction` rolls back instead of becoming a 404). [`lib/tanstack-query/resources/card/`](../lib/tanstack-query/resources/card/) parses a card object (not `| null`). The modal treats a missing card as `isError`, keeps activity on a skeleton until the card succeeds, then hides activity on card error, and remounts header / description / actions / activity when the card id changes ([`card-modal`](../components/modals/card-modal/index.tsx))
- [x] **Org auth DAL** — [`lib/auth/get-org-auth.ts`](../lib/auth/get-org-auth.ts) centralizes org-scoped authentication (wraps Clerk `auth()`). All Server Actions use it via `createSafeAction`; Route Handlers and layouts call it directly. Tests mock `getOrgAuth` instead of `auth`
- [ ] **Stronger DAL / DTO** — only if authorization-near-data keeps getting duplicated beyond `getOrgAuth`; don’t invent `dal.ts` for fashion (see [DAL and DTO](#dal-and-dto-not-auth-only))
...
| Same server helper called twice in one request                                    | Wrap with React [`cache`](https://react.dev/reference/react/cache) (Next’s DAL examples do this) — [Sharing data](https://nextjs.org/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache) | [`checkSubscription`](../lib/subscription.ts) (org page `isPro` + create-board tile remaining copy). Request-scoped only — not Redis, not a Server Action. |
...
| Client needs to load/refetch after mount (modal, poll, dependent on client state) | **TanStack Query** + Web `fetch` (`lib/tanstack-query/fetcher.ts`) → usually our **Route Handler**                                                                                                                    | Card modal → `/api/cards/...`                                                                                                                              |
...
| Whether the write is allowed                                 | Re-check **on the server, on that write** (session, `orgId`, ownership, plan / caps). Do not trust RSC props or client UI as the lock — [UI may be stale; writes re-check](#ui-may-be-stale-writes-re-check)                                                                                                                                                                                                                                                                                 | `createBoard` / `deleteBoard` re-read Pro inside `$transaction`                                                                                     |
| Shape of an Action's input                                   | A **typed object** the client assembles (`execute({ … })`), so the schema validates real data. `FormData` earns its place for genuinely uncontrolled inputs, file uploads, dynamic field sets, or a form that must work unhydrated. Never pack several values into one delimited string. When reading `FormData` into that object, **narrow** via [`formDataString`](../lib/form-data.ts) (or `typeof x === "string" ? x : ""`) — do **not** `as string` (`get` is `string \| File \| null`) | `create-board`: controlled `title` + `image` from [`FormPicker`](../components/form/form-picker.tsx); board/list/card forms use `formDataString`    |
...
| Optimistic UI **before** the Action finishes                 | React [`useOptimistic`](https://react.dev/reference/react/useOptimistic) + Server Actions — [Client mirrors after mutations](#client-mirrors-after-server-actions-not-always-optimistic); Query optimistic only where Query already owns the cache                                                                                                                                                                                                                                           | When needed — list DnD TODO in [`lists-container/index.tsx`](<../app/(platform)/(dashboard)/board/[boardId]/_components/lists-container/index.tsx>) |
| After success, apply Action `data` to **local** Client state | Confirmed local mirror (not classic optimistic) — same section                                                                                                                                                                                                                                                                                                                                                                                                                               | Board / list title forms today                                                                                                                      |
...
| After success, refresh **TanStack Query** cache              | `queryClient.invalidateQueries` on the client using keys from [`lib/tanstack-query/resources/`](../lib/tanstack-query/resources/) factories                                                                                                                                                                                                                                                                                                                                                  | Card modal → one `cardQueries.byId(id)` (prefix-scopes `.detail` + `.auditLogs`)                                                                    |
...
### UI may be stale; writes re-check
...
### Client mirrors after Server Actions (not always “optimistic”)

Newcomers often mix three things: **Server Components**, **Server Actions** (Server Functions called from the client), and **optimistic UI**. This repo’s title forms show why that mix-up hurts.

**Mental model**

1. A **Server Component** (e.g. board page) loads the board and passes it as props into a **Client Component** (`BoardTitleForm`).
2. The Client Component may keep a **local copy** in `useState(data.title)` so it can edit/display without waiting on the server for every keystroke.
3. A **Server Action** (`updateBoard`) runs on the server, writes Prisma, then calls `revalidatePath` so the **next** server render of that route can see the new title.
4. `revalidatePath` does **not** instantly rewrite the Client Component’s props mid-interaction. The parent may re-render later with new `data`; that still **does not remount** the child, so `useState(initial)` does **not** re-run — React keeps the existing state.

**Uncontrolled inputs make this sharper:** `defaultValue={title}` means typing updates the DOM, not React state. After the user types a new name, local `title` state is often still the **pre-edit** value until you `setTitle` yourself.

**Two update timings (use the right word)**

| Timing                     | When local UI changes                                                          | React tool (direction of travel)                                                                                                                  | In this repo today                                                                                                                                                                                                                                           |
| -------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Confirmed local mirror** | **After** the Action returns `{ data }` (`onSuccess`)                          | Plain `setState` from Action `data`                                                                                                               | [`board-title-form.tsx`](<../app/(platform)/(dashboard)/board/[boardId]/_components/board-navbar/board-title-form.tsx>), [`list-header.tsx`](<../app/(platform)/(dashboard)/board/[boardId]/_components/lists-container/list-header.tsx>), card modal header |
| **Optimistic UI**          | **Before** the Action finishes (show the hoped-for result; roll back on error) | Prefer [`useOptimistic`](https://react.dev/reference/react/useOptimistic) with Server Actions ([Forms](https://nextjs.org/docs/app/guides/forms)) | Not adopted yet — see TODO below; list DnD is a candidate                                                                                                                                                                                                    |

Do **not** call the confirmed-mirror pattern “optimistic” in new code/comments — it waits for success. It only feels instant because you apply Action `data` **without waiting for RSC props** to catch up.

**Bug pattern to avoid:** in `onSuccess`, `setTitle(title)` where `title` is the **stale** React state (or the props board), instead of `setTitle(data.title)` where `data` is the **Action result**. The callback parameter often shadows the props name `data` — rename one of them (`result` / `board`) when editing for clarity.

**When you add real optimistic UI later**

- Prefer **`useOptimistic`** for Server Action flows (pending rename, DnD reorder) so React can reconcile with the eventual server tree.
- Prefer **TanStack Query optimistic updates** only where Query already owns the cache (card modal), not as a second pattern for RSC-fed forms — [one tool per job](./vocabulary.md#one-tool-per-job) · catalog in [`conventions.md`](./conventions.md).
- Always plan **rollback** on `serverError` / failed Action; optimistic UI without error UX lies to the user.
- Keep **authorization and persistence** in the Server Action; optimistic UI is display-only until the Action succeeds.

Official depth: [Mutating Data](https://nextjs.org/docs/app/getting-started/mutating-data) · [Server Actions](https://nextjs.org/docs/app/guides/server-actions) · React [`useOptimistic`](https://react.dev/reference/react/useOptimistic).

### TanStack Query (client only)

**Yes — it is part of this repo’s data story**, but only for **browser islands** that must load/refetch after mount. Default page data stays Server Components + Prisma; most form saves stay Server Actions. Do **not** invent a separate `tanstack-query.md` until Query-specific guidance outgrows this section + the catalog row (same bar as Clerk/Prisma getting their own pages).
...
| **How** (`useQuery`, keys, staleTime, …)                             | [TanStack Query docs](https://tanstack.com/query/latest/docs/framework/react/overview) · [queryOptions](https://tanstack.com/query/v5/docs/framework/react/guides/query-options)                                                                                                                                                                                                                                                 |
| **Transport**                                                        | Web `fetch` via [`lib/tanstack-query/fetcher.ts`](../lib/tanstack-query/fetcher.ts) — not Next’s server `fetch`                                                                                                                                                                                                                                                                                                                  |
| **Client factories** (keys + `queryFn`, later Query mutations)       | [`lib/tanstack-query/resources/<resource>/`](../lib/tanstack-query/resources/) — SoT for layout below. **Word “factory”:** [`vocabulary.md`](./vocabulary.md) (TanStack does not define it; official helper is [`queryOptions`](https://tanstack.com/query/v5/docs/framework/react/guides/query-options))                                                                                                                        |
| **Folder map** (`lib/tanstack-query` vs `app/api`, avoid `queries/`) | [`project-structure.md`](./project-structure.md) (paths only — **why** stays here)                                                                                                                                                                                                                                                                                                                                               |
| **Devtools** (local cache / refetch)                                 | [`ReactQueryDevtools`](https://tanstack.com/query/v5/docs/framework/react/devtools) in [`providers/query-provider.tsx`](../providers/query-provider.tsx). Always mount; the default import is a no-op unless `NODE_ENV === 'development'`. Do not wrap with `isDevelopment` (that is Zustand’s `enabled` pattern in [`client-ui-state.md`](./client-ui-state.md)) and do not import `@tanstack/react-query-devtools/production`. |

#### Newcomer path (card modal)

1. **UI intent** lives in Zustand — which card id is open (`stores/use-card-modal-store.ts`). Not the card payload. → [`client-ui-state.md`](./client-ui-state.md)
2. **Remote data** loads with TanStack Query via a **resource factory** — [`lib/tanstack-query/resources/card/`](../lib/tanstack-query/resources/card/) (`cardQueries.detail` / `cardQueries.auditLogs`) → our Route Handlers under `app/api/cards/…` today (same pattern later for other origins).
3. **Writes** stay Server Actions (`actions/update-card`, …). On success, the client calls `queryClient.invalidateQueries({ queryKey: cardQueries.… })` so the modal cache refreshes. RSC pages still use `revalidatePath` inside the Action when needed — different cache ([above](#cache-means-different-things-traditional-be-vs-next-vs-client)).
4. Provider: [`providers/query-provider.tsx`](../providers/query-provider.tsx) via [`createQueryClient`](../lib/tanstack-query/client.ts) (skip 4xx except 408/429, and Zod retries). Same file mounts [`ReactQueryDevtools`](https://tanstack.com/query/v5/docs/framework/react/devtools) (`initialIsOpen={false}`) so local cache and refetch are inspectable — the package no-ops outside development.

#### Why `lib/tanstack-query/resources/<resource>/` (and not the alternatives)

| Choice                                                           | Use it?                  | Why                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------------------------------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`lib/tanstack-query/resources/card/` + `cardQueries` factory** | **Yes**                  | Common practice for Query: one module per remote **resource**, co-locating `queryKey` + `queryFn` (and later Query mutations). Works for **any origin** (our BFF, other services we own, third parties) — not “Next server only.” What “factory” means: [`vocabulary.md`](./vocabulary.md). Official co-locate helper: [`queryOptions`](https://tanstack.com/query/v5/docs/framework/react/guides/query-options). |
| Factories under `components/modals/card-modal/`                  | **No**                   | Modal is UI chrome; keys/invalidation are about the **card** resource and are shared beyond one folder.                                                                                                                                                                                                                                                                                                           |
| Global `queries/` + `mutations/` + `keys/` trees                 | **No**                   | Separating keys from `queryFn` fights [`queryOptions`](https://tanstack.com/query/v5/docs/framework/react/guides/query-options) / TkDodo guidance. A top-level `mutations/` also reads as “all writes go through Query,” which fights our default **Server Actions** write path.                                                                                                                                  |
| Folder named `queries/` only                                     | **No**                   | Too narrow once Query mutations exist; we still want one resource file for reads + cache-touching writes.                                                                                                                                                                                                                                                                                                         |
| `lib/tanstack-query/resources/` vs `app/api/`                    | **Both, different jobs** | `app/api/` = Next **Route Handlers** (HTTP we expose). `lib/tanstack-query/resources/` = **client** factories that call remotes (often our handlers, sometimes not).                                                                                                                                                                                                                                              |
| `services/` as a parallel tree                                   | **No**                   | Same job as `lib/tanstack-query/resources/` — [one tool / one folder per job](./vocabulary.md#one-tool-per-job).                                                                                                                                                                                                                                                                                                  |
...
**Typing:** pass a Zod schema into `fetcher(url, schema)` so `queryFn` returns domain types (`Date`, Prisma enums). `JSON.parse` cannot produce Prisma `GetPayload` / `Date` — don’t type `fetcher` as those. `useQuery` infers `data` from `queryOptions`; don’t duplicate the generic unless you need an override.

**Example in tree today:** [`lib/tanstack-query/`](../lib/tanstack-query/) (`client.ts`, `fetcher.ts`, [`resources/card/`](../lib/tanstack-query/resources/card/)). Provider: `providers/query-provider.tsx`.
...
| -------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Prisma / DB roundtrips** | Server  | **Not** Next’s Data Cache. Same query in two Server Components can hit the DB twice unless you dedupe with React [`cache`](https://react.dev/reference/react/cache) or share one await higher in the tree — we wrap [`checkSubscription`](../lib/subscription.ts) |
...
**Server `fetch` ≠ `lib/tanstack-query/fetcher.ts`:** Next’s extended `fetch` is for the **server**; the client helper is ordinary Web `fetch` for Query — see [`lib/tanstack-query/fetcher.ts`](../lib/tanstack-query/fetcher.ts).
...
No formal `lib/dal.ts`. Authorization + Prisma live mainly in **Server Actions** / Route Handlers / helpers (`orgId` in `where`) — Next’s “component-level data access” path. Client Query factories live under [`lib/tanstack-query/resources/`](../lib/tanstack-query/resources/) ([above](#tanstack-query-client-only)); optional `repositories/` only [when needed](./project-structure.md#common-practice-folders).
...
Server Actions are reachable by POST (not only your UI). Re-check authentication and authorization inside them — [UI may be stale; writes re-check](#ui-may-be-stale-writes-re-check) · [`authentication-and-authorization.md`](./authentication-and-authorization.md) · [Data Security](https://nextjs.org/docs/app/guides/data-security).
