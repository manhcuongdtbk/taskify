...
- **Server data fetching** uses Next’s [extended `fetch`](https://nextjs.org/docs/app/api-reference/functions/fetch) when we call remote/HTTP APIs from Server Components (caching / `revalidate` / `tags`). **Client** TanStack Query uses Web `fetch` via `lib/tanstack-query/fetcher.ts` — do not confuse the two
...
3. **No hand-rolled `BoardIdPageProps`** — the global helper _is_ the props type. Route export **names** still follow [`conventions.md`](./conventions.md#route-mirrored-pagelayout-names) (`BoardIdPage`, `BoardIdLayout`, …). HTTP method names on `route.ts` stay `GET` / `POST` / … (framework-required).
...
**Enforcement:** `scripts/check-route-export-names.ts` via `pnpm lint:routes` (also under `pnpm lint` and lint-staged on `app/**/{page,layout,route}.*`). Autofix: `pnpm lint:routes:fix` (also under `pnpm lint:fix`). Rewrites export names, `PageProps` / `LayoutProps`, and `RouteContext` for common shapes. Unusual signatures that can’t be rewritten safely still fail for a manual fix.
...
## Route Handler throws vs HTTP status

Card JSON BFF: [`app/api/cards/[cardId]/route.ts`](../app/api/cards/%5BcardId%5D/route.ts) · audit logs sibling. Stripe webhook keeps a **narrow** `try/catch` ([`app/api/webhook/route.ts`](../app/api/webhook/route.ts)) because invalid `constructEvent` is an expected 400.

**Do not wrap the whole `GET`/`POST` in `try/catch`.** [`notFound()`](https://nextjs.org/docs/app/api-reference/functions/not-found) (and `redirect()`) **throw**. A surrounding catch swallows them — the docs say so for `notFound` / `unauthorized` — so a missing card would never be a 404. The same catch also turns Prisma/`auth()` bugs into a hand-written 500 body and hides the stack.

| What happens                    | How we express it                                                                                                                                                     | What the client sees                    |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| No session / no org             | `return new NextResponse("Unauthorized", { status: 401 })`                                                                                                            | 401 — `fetcher` throws, Query `isError` |
| Card missing (or other org)     | `notFound()` on card detail **and** card audit logs — **documented** for Route Handlers                                                                               | 404 empty body (`notFound` serves 404)  |
| `redirect()`                    | `redirect()` from `next/navigation`                                                                                                                                   | Redirect status                         |
| Prisma down, unexpected `throw` | **Do not catch.** Next’s App Route module rethrows anything that is not a redirect / HTTP-access fallback (`notFound`, and experimental `unauthorized` / `forbidden`) | Typically **500** on the wire           |

**Official vs runtime:** App Router [Error handling](https://nextjs.org/docs/app/getting-started/error-handling) is about **UI** (`error.tsx` / boundaries). It does **not** say “an uncaught `route.ts` throw becomes HTTP 500.” 404 from `notFound()` in a Route Handler **is** documented. The 500 for other throws is **this Next runtime** (`node_modules/next/dist/server/route-modules/app-route/module.js`: catch fallbacks, else `throw err`). The HTTP server then usually finishes the failed handler as 500 — not a JSON body we control.
...
Centralizing path strings / builders (`paths.ts`, `routes.ts`, `links.ts`, …) is a widespread frontend habit. “Builder” here is everyday English (helpers that return paths), not the GoF Builder pattern — [`vocabulary.md`](./vocabulary.md). You’ll see the same _idea_ next to [React Router](https://reactrouter.com/) (path constants + helpers like [`generatePath`](https://api.reactrouter.com/v7/functions/react-router.generatePath.html)) and [Vue Router](https://router.vuejs.org/) (often **named routes** / path constants beside the router config). Some codebases put _every_ URL there; **this repo keeps a slim map** — only paths that need a `typedRoutes` cast — so the file doesn’t look like every link must go through `paths`.

**Do not confuse that habit with those libraries’ route tables.** In React Router / Vue Router, a central file often _registers_ path → component (the router consumes it). In the App Router, the `app/` tree still owns routing; `lib/paths.ts` only **mirrors** cast-needed URLs for typed links / redirects under [`typedRoutes`](https://nextjs.org/docs/app/api-reference/config/typescript#statically-typed-links).

**Enforcement**

| Check                               | How                                                                                                                                     |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Valid in-app routes                 | `typedRoutes` + `pnpm lint:types` (also under `pnpm lint`)                                                                              |
...
| Route Handler throws vs 401 / 404 / 500          | [`notFound()`](https://nextjs.org/docs/app/api-reference/functions/not-found) (404 is documented on Route Handlers) · [`unauthorized()`](https://nextjs.org/docs/app/api-reference/functions/unauthorized) is experimental · [this section](#route-handler-throws-vs-http-status) — do **not** cite [Error handling](https://nextjs.org/docs/app/getting-started/error-handling) for `route.ts` 500s (that page is UI `error.tsx`)                                                                                                                                                                             |
...
| Server `fetch` caching                           | [Caching (previous model)](https://nextjs.org/docs/app/guides/caching-without-cache-components) · [Revalidating](https://nextjs.org/docs/app/getting-started/revalidating) · vocabulary collision: [`data.md`](./data.md#cache-means-different-things-traditional-be-vs-next-vs-client) (not client `lib/tanstack-query/fetcher.ts`)                                                                                                                                                                                                                                                                           |
