...
- [ ] **Authorization in every Server Action and Route Handler** — assume actions are POSTable directly; verify authentication, authorization (active organization + resource ownership), inside each one ([Data security](https://nextjs.org/docs/app/guides/data-security), [Mutating data](https://nextjs.org/docs/app/getting-started/mutating-data)). Detail + Clerk-side notes: [`authentication-and-authorization.md`](./authentication-and-authorization.md) (**TODO — authorization**). Map: [`data.md`](./data.md)
...
- [ ] **Keep `proxy.ts` to optimistic gating** — full session/authorization stays in pages, layouts, and actions; finish Clerk’s guidance on public routes / less middleware-centric checks ([Proxy](https://nextjs.org/docs/app/getting-started/proxy), [`authentication-and-authorization.md`](./authentication-and-authorization.md) authentication TODOs)
