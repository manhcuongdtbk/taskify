...
| Card missing (or other org)     | `notFound()` — **documented** for Route Handlers                                                                                                                      | 404 empty body (`notFound` serves 404)  |
...
Client Query does not distinguish 401 / 404 / 500 — `lib/fetcher.ts` throws on any `!res.ok`.
