...
| **`AGENTS.md` / Cursor rules**    | Thin drift warnings only — tools where training data fights this repo                         | Next App Router in-tree docs · Vitest jsdom vs Browser Mode · MSW HTTP mocks             |
...
| Shared entity test data with **Fishery** (`lib/testing/factories/`)                             | Adopted     | SoT: [`testing.md`](./testing.md) — prefer [Fishery](https://github.com/thoughtbot/fishery) `Factory.define` / `.build` over hand-rolled `make*` or static fixture blobs; not Vitest `test.extend` fixtures; no `@faker-js/faker` until uniqueness/realism hurts                                                                                                                          |
