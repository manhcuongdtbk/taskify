...
| Shared entity test data with **Fishery** (`lib/testing/factories/`)                             | Adopted     | SoT: [`testing.md` → Fishery practices](./testing.md#fishery-practices) — typed fixtures for tests (not Prisma/ORM integration). One file per entity; [Fishery](https://github.com/thoughtbot/fishery) `Factory.define` / `.build`; never nest another entity’s factory in a sibling file; `sequence` ids; build inside each test (ESLint)                                                |
