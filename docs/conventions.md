...
| Practice | Status | Widespread sources |
| -------- | ------ | ------------------ |
| Type App Router page/layout/route props with global **`PageProps` / `LayoutProps` / `RouteContext`** (not handwritten param shapes) | Adopted | [Route Props Helpers](https://nextjs.org/docs/app/getting-started/layouts-and-pages#route-props-helpers) · [Route Context Helper](https://nextjs.org/docs/app/getting-started/route-handlers#route-context-helper) · repo map [`nextjs.md`](./nextjs.md#route-props-helpers-pageprops--layoutprops--routecontext). Enforced by `pnpm lint:routes` |
