...
| **Fishery**                           | `fishery`                          | [github.com/thoughtbot/fishery](https://github.com/thoughtbot/fishery) README for the installed major · concern [`testing.md`](./testing.md) (test data factories under `lib/testing/factories/`)                                               |
...
| **date-fns**                          | `date-fns`                         | [date-fns.org](https://date-fns.org) for the installed major (v4)                                                                                                                                                                               |
...
| Domain entity shared widely                   | Shared module — **not** `card-item.types.ts`                     | `CardWithList` / `ListWithCards` in `lib/prisma/payloads.ts` (`GetPayload` + shared args); Prisma `Board`                                |
...
| Shared domain model                               | `@/lib/prisma/payloads` / Prisma Client types — not per-component `*.types.ts`                                                          |
...
| Co-locate types with the module that owns them; avoid a giant dumping-ground `types/` unless it earns its keep | Adopted     | Common TS habit; Prisma query payloads live in [`lib/prisma/payloads.ts`](../lib/prisma/payloads.ts) — don’t park UI wrapper props there                                                                                                                                                                                                                                                                                                                                           |
...
| Shared entity test data with **Fishery** (`lib/testing/factories/`)                             | Adopted     | SoT: [`testing.md` → Fishery practices](./testing.md#fishery-practices) — one file per entity (`list.ts`, `card.ts`, …); [Fishery](https://github.com/thoughtbot/fishery) `Factory.define` / `.build`; never nest another entity’s factory in a sibling file; `sequence` ids; build inside each test (ESLint)                                                                             |
