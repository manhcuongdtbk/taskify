...
| App Router `page` / `layout` props: global **`PageProps` / `LayoutProps`** with correct route literal | Adopted | [Route Props Helpers](https://nextjs.org/docs/app/getting-started/layouts-and-pages#route-props-helpers) · [`nextjs.md`](./nextjs.md#route-props-helpers-pageprops--layoutprops). Same script (`pnpm lint:routes` / `pnpm lint:routes:fix`) |
...
5. **Do not** invent handwritten `{SameName}Props` for App Router `page` / `layout` — use Next’s global [`PageProps`](./nextjs.md#route-props-helpers-pageprops--layoutprops) / `LayoutProps` helpers instead (see [`nextjs.md`](./nextjs.md)). Enforced by `pnpm lint:routes` (same script as export names). The `noctcore-react/component-props-naming` rule still applies when someone *does* introduce a named props type on `app/**/page.tsx`; prefer not to.
...
| Practice | Status | Widespread sources |
| -------- | ------ | ------------------ |
| Type App Router page/layout props with global **`PageProps` / `LayoutProps`** (not handwritten param shapes) | Adopted | [Route Props Helpers](https://nextjs.org/docs/app/getting-started/layouts-and-pages#route-props-helpers) · repo map [`nextjs.md`](./nextjs.md#route-props-helpers-pageprops--layoutprops). Enforced by `pnpm lint:routes`. **`RouteContext` for `route.ts` is TODO** there |
