...
export const CardModal = () => {
  ...
  const { data: card, isError: isCardError } = useQuery(cardQueries.detail(id));
  const { data: cardAuditLogs, isError: isCardAuditLogsError } = useQuery(
    cardQueries.auditLogs(id),
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg md:max-w-2xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-5xl">
        {isCardError ? (
          <CardModalLoadError>{cardLoadErrorCopy}</CardModalLoadError>
        ) : !card ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 md:gap-4">
          <div className="col-span-3">
            <div className="w-full space-y-6">
              {isCardError ? null : !card ? (
                <CardModalDescription.Skeleton />
              ) : (
                <CardModalDescription card={card} />
              )}
              {isCardAuditLogsError ? (
                <CardModalLoadError>
                  {cardActivityLoadErrorCopy}
                </CardModalLoadError>
              ) : !cardAuditLogs ? (
